...
const EditJobPage = ({updateJobSubmit}) => {
  ...
  const submitForm = (e) => {
     ...
     //We need to match the form data structure to match structure of our API
     const updatedJob = {
       id, //Here we need the in the updatedJob object to perform the update request to the job that is being edited.
       title, //it is writen without key:value because both key and value were same so we can just write single key
       type,
       location,
       description,
       salary,
      ...
      <div className="bg-white px-6 py-8 mb-4 shadow-md rounded-md border m-4 md:m-0">
        <form onSubmit={submitForm}>
          ...
          <div className="mb-4">
            ...
            <input type="text" id="company" name="company" className="border rounded w-full py-2 px-3" placeholder="Company Name" value={companyName} onChange={(e) => setCompanyName(e.target.value)} />
          </div>
          ...
          <div className="mb-4">
            ...
            <textarea id="company_description" name="company_description" className="border rounded w-full py-2 px-3" rows="4" placeholder="What does your company do?" value={companyDescription} onChange={(e) => setCompanyDescription(e.target.value)}></textarea>
          </div>
          ...
          <div className="mb-4">
            ...
            <input type="email" id="contact_email" name="contact_email" className="border rounded w-full py-2 px-3" placeholder="Email address for applicants" required value={contactEmail} onChange={(e) => setContactEmail(e.target.value)} />
          </div>
          <div className="mb-4">
